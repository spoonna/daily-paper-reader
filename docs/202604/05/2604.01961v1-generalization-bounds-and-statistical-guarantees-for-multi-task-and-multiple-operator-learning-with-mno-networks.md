...
motivation: 旨在解决多算子学习在处理分层采样数据时缺乏定量统计泛化保证和样本复杂性分析的问题。
method: 利用覆盖数分析方法，针对MNO架构中深度ReLU子网络的乘积线性组合推导显式的度量熵界限。
result: 推导出了关于算子、输入和评估点采样预算的显式近似-估计权衡公式，并给出了算子采样预算下的学习率。
conclusion: 该研究为MNO作为通用PDE求解器或基础模型的泛化能力提供了严谨的理论框架，明确了跨算子实例的样本复杂性。
---

## 摘要
多算子学习涉及学习由算子描述符 $\alpha$ 索引的算子族 $\{G[\alpha]:U\to V\}_{\alpha\in W}$。训练数据通过分层采样收集：首先采样算子实例 $\alpha$，然后针对每个实例采样输入函数 $u$，最后针对每个输入采样评估点 $x$，从而获得 $G[\alpha][u](x)$ 的带噪声观测值。尽管近期研究已开发出具有强表达能力的多任务与多算子学习架构及逼近论意义下的标度律，但定量的统计泛化保证仍然有限。我们为可分离模型提供了一种基于覆盖数的泛化分析，重点关注多神经网络算子（MNO）架构：我们首先推导了由深层 ReLU 子网络乘积的线性组合所构成的假设类的显式度量熵界，随后将这些复杂度界与 MNO 的逼近保证相结合，从而针对新（未见）三元组 $(\alpha,u,x)$ 的期望测试误差，获得了显式的逼近-估计权衡。所得出的界限使对分层采样预算 $(n_\alpha,n_u,n_x)$ 的依赖关系变得透明，并给出了关于算子采样预算 $n_\alpha$ 的显式学习率表述，为跨算子实例的泛化提供了样本复杂度表征。该结构和架构也可被视为通用求解器或“小型”PDE 基础模型的一个示例，其中三元组是多模态的一种形式。
