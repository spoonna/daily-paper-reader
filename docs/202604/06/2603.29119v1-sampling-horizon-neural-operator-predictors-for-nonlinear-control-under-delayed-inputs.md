...
tags: ["query:dl-pde"]
score: 6.0
evidence: 用于非线性控制中求解隐式常微分方程的神经算子
tldr: 针对具有输入延迟和采样测量的非线性控制系统，本文提出了两种基于神经算子的预测反馈设计。通过引入采样时界预测算子，解决了传统预测器在线求解ODE计算量大以及依赖连续测量的问题。研究证明了系统的半全局实际稳定性，并在6自由度机械臂实验中实现了25倍的计算加速，有效平衡了采样灵活性与近似精度。
source: arxiv
...
motivation: 传统的预测反馈控制在处理输入延迟时面临在线求解ODE计算成本高以及难以应对非连续采样测量的问题。
method: 提出了两种神经算子设计：一种是映射测量与输入历史至采样区间预测轨迹的采样时界算子，另一种是结合闭环流以支持非均匀采样的预测算子。
result: 理论上证明了具有显式误差界的半全局实际稳定性，并在6自由度机械臂仿真中实现了比基准方法快25倍的计算速度。
conclusion: 神经算子能有效替代复杂的在线ODE求解，在保证非线性系统稳定性的同时显著提升了延迟补偿控制的实时性。
---

## 摘要
现代控制系统经常在输入延迟和采样状态测量下运行。一种常见的延迟补偿策略是预测器反馈；然而，实际实现需要在线求解隐式常微分方程（ODE），导致计算成本难以承受。此外，预测器公式通常假设状态测量是连续可用的，而在实践中，测量可能是采样的、不规则的，或者由于硬件故障而暂时缺失。在这项工作中，我们为具有延迟输入和采样测量的非线性系统开发了两种神经营算器预测器反馈设计。在第一种设计中，我们引入了一个采样时界预测算子，它将当前的测量值和输入历史映射到下一个采样间隔内的预测状态轨迹。在第二种设计中，神经营算器仅近似延迟补偿预测器，然后将其与测量之间的闭环流相结合。第一种方法需要均匀采样，但产生的残差界限与算子近似误差直接相关。相比之下，第二种方法以放大近似误差为代价，适应非均匀但有界的采样计划，为控制工程师揭示了采样灵活性与近似敏感性之间的实际权衡。对于这两种方案，我们都建立了具有显式神经营算器误差相关界限的半全局实际稳定性。在 6 连杆非线性机器人操作臂上的数值实验证明了精确的跟踪性能，并且比基准方法实现了 25 倍的显著计算加速。
